...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	СтруктураТаблицПроведения = РаботаСДокументами.ПолучитьСтруктуруТаблицПроведения(ЭтотОбъект);	
	РаботаСДокументами.ДобавитьСтандартныеДвижения(ЭтотОбъект, СтруктураТаблицПроведения);

	Движения.Записать();
	
	РезультатПроверки = РаботаСДокументами.ВыполнитьСтандартныйКонтроль(СтруктураТаблицПроведения, ЭтотОбъект);
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	СтруктураТаблицПроведения = РаботаСДокументами.ПолучитьСтруктуруТаблицПроведения(ЭтотОбъект);	
	РаботаСДокументами.ДобавитьСтандартныеДвижения(ЭтотОбъект, СтруктураТаблицПроведения, Истина);
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("Структура") Тогда		
		 Если ДанныеЗаполнения.Свойство("Товары") И
			 ЗначениеЗаполнено(ДанныеЗаполнения.Товары) И 
			 ТипЗнч(ДанныеЗаполнения.Товары) = Тип("ТаблицаЗначений") Тогда
		 	
		 	Товары.Загрузить(ДанныеЗаполнения.Товары);
		 	
		 КонецЕсли;	 
		  	 	
		 Если ДанныеЗаполнения.Свойство("Номенклатура") И
		 	ЗначениеЗаполнено(ДанныеЗаполнения.Номенклатура) И
		 	ТипЗнч(ДанныеЗаполнения.Номенклатура) = Тип("Массив") И
		 	Не Товары.Количество() Тогда
		 		
		 	  ОбщегоНазначенияКлиентСервер.СвернутьМассив(ДанныеЗаполнения.Номенклатура);		 	  
		 	  ОбщегоНазначенияКлиентСервер.ДополнитьТаблицуИзМассива(Товары, ДанныеЗаполнения.Номенклатура, "Номенклатура");	 
...
Процедура СформироватьТекстЗапросаТовары(ТекстЗапроса, ПараметрыЗапроса)
	...
	ПараметрыЗапроса.Вставить("Товары", Товары.Выгрузить(,"НомерСтроки, Номенклатура, Склад, Подразделение, Количество"));
