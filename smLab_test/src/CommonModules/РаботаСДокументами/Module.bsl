...
Функция ПолучитьСтруктуруТаблицПроведения(ДокументОбъект) Экспорт  
	...
	Если ДокументОбъект.Движения.Найти("ПартииТоваров") <> Неопределено Тогда		
		...
		СтруктураТаблиц.Вставить("КонтрольПартииТоваров", ДокументОбъект.Движения.ПартииТоваров.Выгрузить(,"Номенклатура,ПартияНоменклатуры"));		
	КонецЕсли;	
...
Процедура ДобавитьДвиженияПоступление(ДокументОбъект, СтруктураТаблиц, ТаблицаДанныхДокумента)
	...
	Для Каждого СтрокаТовары Из ТаблицаДанныхДокумента Цикл			
			...
			НовоеДвижение.ПартияНоменклатуры = ДокументОбъект.Ссылка;
			...
			НовыйКонтроль.ПартияНоменклатуры = ДокументОбъект.Ссылка;
	...
	СтруктураТаблиц.ПартииТоваров.Свернуть("Активность,ВидДвижения,Период,Номенклатура,ПартияНоменклатуры","Количество");
	СтруктураТаблиц.КонтрольПартииТоваров.Свернуть("Номенклатура,ПартияНоменклатуры");
...
Процедура ДобавитьДвиженияРасход(ДокументОбъект, СтруктураТаблиц, ТаблицаДанныхДокумента)
	...
	СтруктураТаблиц.ОстаткиТоваров.Свернуть("Активность,ВидДвижения,Период,Организация,Склад,Номенклатура","Количество");
	СтруктураТаблиц.КонтрольОстаткиТоваров.Свернуть("Организация,Склад,Номенклатура");
	СтруктураТаблиц.Продажи.Свернуть("Активность,ВидДвижения,Период,Номенклатура,Подразделение","Количество");	
...
Процедура КонтрольПартийТоваров(ТаблицаКонтроля, РезультатКонтроля, ДокументОбъект = Неопределено)
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втКонтроля КАК втКонтроля
	|		ПО ПартииТоваровОстатки.Номенклатура = втКонтроля.Номенклатура
	...
	Пока Выборка.Следующий() Цикл
		 ...
		 Если ПустаяСтрока(РезультатКонтроля.ТекстОшибки) Тогда
		 	ТекстСообщения = НСтр("ru = 'Номенклатура %1(Партия: %2): недостаточно свободного остатка. (кол. :%3)'");
		 Иначе
		 	ТекстСообщения = Символы.ПС + НСтр("ru = 'Номенклатура %2(Партия: %1): недостаточно свободного остатка. (кол. :%3)'");	
		 КонецЕсли;	
